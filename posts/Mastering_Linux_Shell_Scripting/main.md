...
$ awk '/[[:upper:]]/{print $0}' myfile4
This LINE contains upper case.
ALSO this one.

$ sed -n '/[[:upper:]]/p' myfile4
This LINE contains upper case.
ALSO this one.
```
### 11.2.6　アスタリスク
アスタリスクは、ある文字や文字クラスが0個以上存在するものにマッチする。
## 11.3　EREパターンの定義
EREパターンは、BREパターンの他に次のパターンを理解する。
- 疑問符
- プラス記号
- 波括弧
- パイプ文字
...
AWKはデフォルトでEREパターンをサポートしており、sedは -rオプション が必要。
### 11.3.1　疑問符
前の文字または文字クラスが0個または1個のみ存在するものにマッチする。
### 11.3.2　プラス記号
前の文字または文字クラスが1個以上存在するものにマッチする。
### 11.3.3　波括弧
前の文字または文字クラスが存在する個数を定義する。
### 11.3.4　パイプ文字
正規表現エンジンに渡された文字列のいずれかにマッチするよう指示。
### 11.3.5　正規表現のグループ化
丸括弧（）を使って文字や単語をグループ化し、正規表現エンジンの視点からすると、それらが1つに見えるようにする。
## 11.4　grepの使用
grepツールは、デフォルトではBREパターンを理解する。EREパターンを使いたい場合は、-Eオプションを用いる。
## 11.5　まとめ
## 11.6　練習問題
...
- 特定のフィールドにマッチさせる場合には、awkのマッチ演算子チルダ（~）を用いる。
```
